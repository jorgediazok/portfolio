...
- **Resume download**: the Navbar's "Resume"/"Currículum" button links to `RESUME_LINKS[locale]` (`utils/resume.ts`), which reads `NEXT_PUBLIC_RESUME_URL_EN` / `NEXT_PUBLIC_RESUME_URL_ES` from the environment — deliberately **not** hardcoded, so the actual Google Drive links never sit in the public git history. Set both in `.env.local` for local dev (gitignored) and in Netlify's Site settings → Environment variables for production; since these vars are read at build time for the statically-generated `/en` and `/es` pages, a missing/changed value requires a rebuild (redeploy) to take effect, not just a page refresh. The links use Drive's `uc?export=download&id=...` format, which returns the file directly (`Content-Disposition: attachment`) rather than Drive's preview page.
